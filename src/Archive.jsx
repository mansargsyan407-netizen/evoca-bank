...
const Archive = () => {
  // Առաջին բաժինը (index 0) բաց է ըստ նկարի
  ...
  const sections = [
    {
      ...
      content: (
        <div className="pt-3 space-y-3">
          <div className="font-bold text-purple-900 text-sm">2024</div>
          <a
            href="#"
            className="inline-block text-purple-700 underline hover:text-purple-900 text-xs font-semibold"
          >
            Կանխիկ մուտքի համար Բանկի կողմից սահմանված դրույքաչափեր (ՌԴ ռուբլի RUB, Բրիտանական ֆունտ GBP, Շվեյցարական ֆրանկ CHF)
          </a>
        </div>
      ),
    },
    {
      ...
      content: <div className="pt-2 text-gray-500">Տվյալները հասանելի չեն:</div>,
    },
    {
    ...
    <div className="bg-gray-50 min-h-screen text-gray-800 font-sans pb-16">
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 pt-6">
        <div className="text-xs text-gray-500 mb-6 flex items-center space-x-2">
          ...
          <span>›</span>
          <span className="text-gray-800 font-medium">Արխիվ</span>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 space-y-8">
        <div>
          <h2 className="text-xs font-bold tracking-wider text-gray-900 uppercase mb-4">
              ...
              return (
                <div
                  ...
                  className={`border rounded-2xl transition-all duration-200 overflow-hidden bg-white ${
                    isOpen ? 'border-purple-300 shadow-sm' : 'border-gray-200'
                  }`}
                >
                  <button
                    onClick={() => toggleAccordion(idx)}
                    className="w-full flex justify-between items-center px-6 py-4 text-left font-bold text-xs md:text-sm text-gray-900 hover:text-purple-900 focus:outline-none"
                  >
                    <span>{sec.title}</span>
                    <span className="text-purple-700 text-xs ml-4">
                      {isOpen ? '▲' : '▼'}
                    </span>
                  </button>

                  {isOpen && (
                    <div className="px-6 pb-5 border-t border-gray-100 text-xs">
                      {sec.content}
                    </div>
                  )}
                </div>
        ...
        <div>
          ...
          <div className="bg-purple-50/60 rounded-xl p-4 flex items-center space-x-3 text-xs text-gray-800 font-semibold border border-purple-100 cursor-pointer hover:bg-purple-100/50 transition">
            <div className="p-2 bg-purple-100 rounded-lg text-purple-700 font-bold">
              📄 ⬇
            </div>
            <span>Տեղեկատվական ամփոփագիր (Վարկային պրոդուկտներ) 07.04.2026</span>
          </div>
        </div>
