...
export default function RoadPolicePaymentsPage() {
    ...
    <div className="min-h-screen bg-[#f8f7fc] text-gray-800 font-sans flex flex-col justify-between">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 w-full">
        ...
        <nav className="flex items-center gap-2 text-xs sm:text-sm mb-8 justify-center sm:justify-start">
          ...
          <span className="text-gray-500 font-medium">ՃՈ վճարներ</span>
        </nav>
        ...
        {/* Police Services Cards Grid */}
        <div className="flex justify-center max-w-4xl mx-auto">
          {policeServices.map((item) => (
            <Link
              key={item.id}
              to={item.link}
              className="w-full max-w-xs flex flex-col items-center justify-center p-8 rounded-3xl cursor-pointer min-h-[220px] text-center bg-white border border-transparent transition-all duration-300 hover:bg-[#e8e0fc] hover:border-[#8c46ff] hover:shadow-md"
            >
