...
export default function EvocabankServicesPage() {
  const services = [
    {
      id: 'loan_repayment',
      title: 'Վարկի մարում',
  ...
  return (
    <div className="min-h-screen bg-[#f8f7fc] text-gray-800 font-sans flex flex-col justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="text-gray-500 hover:text-gray-800">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <path d="M2 12h20M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10z" strokeWidth="1.5" />
              </svg>
            </button>
      ...
      <main className="max-w-5xl mx-auto px-4 py-10 w-full">
        ...
        <nav className="flex items-center gap-2 text-sm mb-12 justify-center sm:justify-start">
          <Link to="/online-payments" className="text-[#6c00ff] font-semibold hover:underline">
            Գլխավոր
          </Link>
          <span className="text-gray-400">/</span>
          <span className="text-gray-500 font-semibold">EVOCABANK</span>
        </nav>

        <h1 className="text-3xl sm:text-4xl font-extrabold text-center text-[#6c00ff] mb-12 tracking-wide uppercase">
          EVOCABANK
        </h1>

        {/* 3 Main Cards with Hover Effect */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {services.map((item) => (
            <div
              key={item.id}
              className="flex flex-col items-center justify-center p-8 rounded-3xl cursor-pointer min-h-[220px] text-center bg-white border border-transparent transition-all duration-300 hover:bg-[#e8e0fc] hover:border-[#8c46ff] hover:shadow-md"
            >
      ...
      {/* Footer */}
      <footer className="bg-white border-t border-gray-100 py-8 px-4 sm:px-8">
