import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { Swiper, SwiperSlide } from 'swiper/react';
import { Navigation } from 'swiper/modules';

// Swiper CSS
import 'swiper/css';
import 'swiper/css/navigation';

const cardsData = [
 {
    id: 'travel',
    title: 'Evoca Travel Card',
    imgUrl: 'https://www.evoca.am/images-cache/sliders/1/17480089224912/4012c7541d8db15b5666bb0e4f4bdf7a-576x486.png',
    link: '/cards/travel-card'
  },
  {
    id: 'visa-platinum',
    title: 'Evoca Visa Platinum',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17798007931247/415x261.png',
    link: '/cards/visa-platinum'
  },
  {
    id: 'wilco-infinite',
    title: 'Wilco Visa Infinite',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17815131185095/415x261.png',
    link: '/cards/wilco-visa-infinite'
  },
  {
    id: 'gift-card',
    title: 'Evoca Gift Card',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17767720288483/415x261.png',
    link: '/cards/gift-card'
  },
  {
    id: 'gift-4u',
    title: 'Digital Gift Card',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17282986912132/415x261.png',
    link: '/cards/4u-gift-card'
  },
  {
    id: 'mc-gold',
    title: 'Mastercard Gold',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17149865321136/415x261.png',
    link: '/cards/mastercard-gold'
  },
  {
    id: 'visa-infinite',
    title: 'Visa Infinite',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/1772717001933/415x261.png',
    link: '/cards/visa-infinite'
  },
  {
    id: 'visa-vision',
    title: 'Visa Vision',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/1714986482757/415x261.png',
    link: '/cards/visa-vision'
  },
  {
    id: 'mc-world-digital',
    title: 'Mastercard World Digital',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17639683196125/415x261.png',
    link: '/cards/mastercard-world-digital'
  },
  {
    id: 'unionpay-business-platinum',
    title: 'UnionPay Business Platinum',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17249401821904/415x261.png',
    link: '/cards/unionpay-business-platinum'
  },
  {
    id: 'myler-gift-card',
    title: 'MyLer Gift Card',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17655348192361/415x261.png',
    link: '/cards/myler-gift-card'
  },
  {
    id: 'unionpay-gold',
    title: 'UnionPay Gold',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17262129422977/415x261.png',
    link: '/cards/unionpay-gold'
  },
  {
    id: 'arca-classic',
    title: 'Arca Classic',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17485032554482/415x261.png',
    link: '/cards/arca-classic'
  },
  {
    id: 'arca-unionpay',
    title: 'Arca Union Pay Co-badge',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17149865321136/415x261.png',
    link: '/cards/arca-unionpay'
  },
  {
    id: 'mc-standard',
    title: 'Mastercard Standard',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17149866652788/415x261.png',
    link: '/cards/mastercard-standard'
  },
  {
    id: 'visa-digital',
    title: 'Visa Digital',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17485025148319/415x261.png',
    link: '/cards/visa-digital'
  },
  {
    id: 'visa-classic',
    title: 'Visa Classic',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/1714986642953/415x261.png',
    link: '/cards/visa-classic'
  },
  {
    id: 'arca-classic-page',
    title: 'Arca Classic',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17404717644263/415x261.png',
    link: '/cards/arca-classic-card'
  },
  {
    id: 'arca-unionpay-cobadge',
    title: 'Arca Union Pay Co-badge',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17881574661708/415x261.png',
    link: '/cards/arca-union-pay-co-badge'
  },
  /* --- Վերջին 4 քարտերը փոխված link-երով --- */
  {
    id: 'visa-business-premium',
    title: 'Visa Business',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17149865475676/415x261.png',
    link: '/cards/visa-business-card'
  },
  {
    id: 'dalma-gift-card-page',
    title: 'Dalma Gift Card',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17404717113297/415x261.png',
    link: '/cards/dalma-card'
  },
  {
    id: 'rio-gift-card-page',
    title: 'Rio Gift Card',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17404717289057/415x261.png',
    link: '/cards/rio-card'
  },
  {
    id: 'visa-gold-premium',
    title: 'Visa Gold',
    imgUrl: 'https://www.evoca.am/images-cache/cards/1/17149865646885/415x261.png',
    link: '/cards/visa-gold-card'
  }
];

const PurpleCard = () => {
  const [activeTab, setActiveTab] = useState('about');
  const [currency, setCurrency] = useState('AMD');

  return (
    <div className="min-h-screen bg-white text-gray-800 font-sans">
      {/* 1. Header Navigation */}
      <header className="border-b border-gray-100 text-xs">
       

       

        <div className="bg-purple-700 text-white text-xs">
          <div className="max-w-7xl mx-auto px-4 flex gap-6">
            <button className="bg-purple-900 px-4 py-2.5 font-bold">Քարտեր</button>
            <button className="hover:bg-purple-800 px-3 py-2.5">Քարտերի տրամադրում և սպասարկում</button>
            <button className="hover:bg-purple-800 px-3 py-2.5">Սոցիալական ապահովության վճարային քարտեր</button>
            <button className="hover:bg-purple-800 px-3 py-2.5">Evoca Benefits</button>
          </div>
        </div>
      </header>

      {/* 2. Hero Section */}
      <section className="bg-gradient-to-b from-purple-50/50 to-white py-12 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 items-center gap-8">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4">
              Mastercard Standard
            </h1>
            <p className="text-gray-600 text-xs leading-relaxed max-w-sm">
              Աշխարհի ցանկացած կետում, որտեղ էլ լինես, քո ֆինանսական միջոցները 24/7 սկզբունքով հասանելի կլինեն:
            </p>
          </div>
          <div className="flex justify-center md:justify-end">
         
             
             
              <div className="flex justify-between items-end">
               
             <img src="https://www.evoca.am/images-cache/cards/1/17149866652788/415x261.png" alt="" />
              </div>
           
          </div>
        </div>
      </section>

      {/* Breadcrumb Navigation */}
     <div className="max-w-6xl mx-auto px-6 py-6 flex flex-wrap items-center justify-between gap-4 text-xs text-gray-500">
              <Link 
                to="/" 
                className="inline-flex items-center gap-2 px-4 py-1.5 border border-gray-200 rounded-full hover:bg-gray-50 transition text-gray-700 font-medium"
              >
                <span>←</span> Վերադառնալ
              </Link>
    
              <div className="flex items-center gap-2">
                <Link to="/" className="hover:underline">🏠</Link>
                <span>›</span>
                <span className="hover:underline cursor-pointer">Անհատ</span>
                <span>›</span>
                <span className="hover:underline cursor-pointer">Քարտեր</span>
                <span>›</span>
                <span className="hover:underline cursor-pointer">Քարտեր</span>
                <span>›</span>
                <span className="hover:underline cursor-pointer">Նվեր քարտեր</span>
                <span>›</span>
                <span className="text-gray-900 font-semibold">Digital Gift Card</span>
              </div>
            </div>
    

      {/* 3. Main Content Area */}
      <main className="max-w-7xl mx-auto px-4 py-6">
        {/* Navigation Tabs */}
        <div className="border-b border-gray-200 mb-8 flex gap-8">
          <button
            onClick={() => setActiveTab('about')}
            className={`pb-3 text-sm font-bold relative transition ${
              activeTab === 'about' ? 'text-purple-700 border-b-2 border-purple-700' : 'text-gray-500 hover:text-gray-800'
            }`}
          >
            Քարտի մասին
          </button>
          <button
            onClick={() => setActiveTab('rates')}
            className={`pb-3 text-sm font-bold relative transition ${
              activeTab === 'rates' ? 'text-purple-700 border-b-2 border-purple-700' : 'text-gray-500 hover:text-gray-800'
            }`}
          >
            Սակագներ և դրույքներ
          </button>
        </div>

        {/* TAB 1: Քարտի մասին */}
        {activeTab === 'about' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div className="space-y-4 text-xs text-gray-700 leading-relaxed">
              <p>
                <strong className="text-purple-700 font-bold">Evocabank</strong>-ը, կարևորելով իր հաճախորդների դրամական միջոցների ապահովությունն ու անվտանգությունը, առաջարկում է <strong className="text-purple-700 font-bold">Mastercard Standard</strong> վճարային քարտեր՝ հնարավորություն տալով կենտրոնանալ կյանքի առավել կարևոր իրադարձությունների վրա:
              </p>
              <p>
                <strong className="text-purple-700 font-bold">Mastercard Standard</strong> քարտը հանդիսանում է <span className="text-purple-700 font-semibold underline">Mastercard միջազգային վճարահաշվարկային համակարգի չիպային քարտ</span>: Այն օժտված է ժամանակակից տեխնոլոգիաներով և ապահովված է դրամական միջոցների անվտանգության ապահովման բարձր մակարդակով: Քարտը հնարավոր է օգտագործել ինչպես ՀՀ տարածքում, այնպես էլ ամբողջ աշխարհում:
              </p>
              <p>
                Դու կկարողանաս քո <strong className="text-purple-700 font-bold">Mastercard Standard</strong> քարտից գումար կանխիկացնել, անկանխիկ վճարումներ կատարել բոլոր այն սպասարկման և առևտրի կետերում, որտեղ փակցված կլինի Mastercard տարբերանշանը:
              </p>
            </div>

            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
              <div className="flex gap-2 justify-start mb-6">
                <button onClick={() => setCurrency('AMD')} className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center ${currency === 'AMD' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600'}`}>֏</button>
                <button onClick={() => setCurrency('USD')} className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center ${currency === 'USD' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600'}`}>$</button>
                <button onClick={() => setCurrency('EUR')} className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center ${currency === 'EUR' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600'}`}>€</button>
                <button onClick={() => setCurrency('RUB')} className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center ${currency === 'RUB' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-600'}`}>₽</button>
              </div>

              <div className="space-y-5 text-xs">
                <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                  <span className="text-2xl font-bold text-purple-700 w-28">0%</span>
                  <span className="text-gray-600 flex-1">Կանխիկացում բանկի կանխիկացման կետերում մինչև 1 մլն ֏</span>
                </div>
                <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                  <span className="text-2xl font-bold text-purple-700 w-28">0.8%</span>
                  <span className="text-gray-600 flex-1">Կանխիկացում ԱրՔա անդամ բանկերի կանխիկացման կետերում</span>
                </div>
                <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                  <div className="w-28">
                    <span className="text-[9px] text-gray-400 block font-sans">min 1,500 ֏</span>
                    <span className="text-2xl font-bold text-purple-700">1%</span>
                  </div>
                  <span className="text-gray-600 flex-1">Կանխիկացում ԱրՔա անդամ չհանդիսացող բանկերի կանխիկացման կետերում</span>
                </div>
                <div className="flex items-center justify-between pb-4 border-b border-gray-100">
                  <span className="text-2xl font-bold text-purple-700 w-28">5000 ֏</span>
                  <span className="text-gray-600 flex-1">Քարտի տարեկան սպասարկում</span>
                </div>
                <div className="flex items-center justify-between pt-1">
                  <span className="text-2xl font-bold text-purple-700 w-28">15,000 ֏</span>
                  <span className="text-gray-600 flex-1">Տարեկան սպասարկում օտարերկրյա քաղաքացիների համար</span>
                </div>
              </div>
            </div>
          </div>
        )}

        {/* TAB 2: Սակագներ և դրույքներ (Ամբողջական Աղյուսակներ) */}
        {activeTab === 'rates' && (
          <div className="space-y-10 text-xs text-gray-800">
            {/* Table 1: Վճարային քարտեր */}
            <div className="overflow-x-auto rounded-2xl border border-gray-200/80 shadow-sm bg-white">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-purple-50/50 text-purple-900 border-b border-gray-200 font-bold">
                    <th colSpan="5" className="py-3 px-4 text-center text-sm font-extrabold border-b border-gray-200">
                      Վճարային քարտեր <sup>[1]</sup>
                    </th>
                  </tr>
                  <tr className="bg-gray-50 text-gray-700 border-b border-gray-200">
                    <th className="py-3 px-4 w-1/3 border-r border-gray-200">Մատուցվող ծառայություններ</th>
                    <th className="py-3 px-4 border-r border-gray-200">MasterCard Standard / Visa Classic</th>
                    <th className="py-3 px-4 border-r border-gray-200">MasterCard Gold / Visa Gold <sup>[2]</sup></th>
                    <th className="py-3 px-4">Visa Business</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Քարտի տրամադրում</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="2">Քարտի սպասարկում</td>
                    <td className="py-3 px-4 border-r border-gray-200"><span className="text-gray-500 mr-2">Տարեկան</span> 5,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200"><span className="text-gray-500 mr-2">Տարեկան</span> 15,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4"><span className="text-gray-500 mr-2">Տարեկան</span> 10,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">Ամսական</span>
                      MasterCard Standard 500 ՀՀ դրամ (Ամսական սպասարկման վճարով քարտի պատվեր հնարավոր է իրականացնել միայն <span className="text-purple-700 underline cursor-pointer font-semibold">Բանկի մասնաճյուղերում</span>)
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">Ամսական</span>
                      MasterCard Gold 1,500 ՀՀ դրամ (Ամսական սպասարկման վճարով քարտի պատվեր հնարավոր է իրականացնել միայն <span className="text-purple-700 underline cursor-pointer font-semibold">Բանկի մասնաճյուղերում</span>)
                    </td>
                    <td className="py-3 px-4 text-gray-400">-</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Շտապ թողարկում</td>
                    <td className="py-3 px-4 border-r border-gray-200">10,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">10,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">10,000 ՀՀ դրամ</td>
                  </tr>
                  <tr className="bg-gray-50/50">
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      24.02.2022թ.-ից հետո Բանկի հաճախորդ դարձած օտարերկրյա ոչ ռեզիդենտ քաղաքացիների համար: <br/>
                      <span className="text-[11px] text-gray-500 font-normal">Visa Business քարտի դեպքում օտարերկրյա քաղաքացիություն ունեցող Հայաստանում գրանցված անհատ ձեռնարկատերերի և իրավաբանական անձանց քարտի տարեկան սպասարկում <sup>[3]</sup></span>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200 font-bold text-purple-900">15,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200 font-bold text-purple-900">45,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 font-bold text-purple-900">30,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Կից քարտի տրամադրում</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Կից քարտի տարեկան սպասարկում <sup>[4]</sup></td>
                    <td className="py-3 px-4 border-r border-gray-200">3,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">10,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">7,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Քարտային հաշվի չնվազող մնացորդ</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr className="bg-gray-50/30">
                    <td colSpan="4" className="py-2 px-4 font-bold text-purple-800 bg-purple-50/30 border-b border-gray-200">
                      Քարտային հաշվի դրական մնացորդի նկատմամբ հաշվարկվող տարեկան %
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">Մինչև 5 մլն. ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">0 %</td>
                    <td className="py-3 px-4 border-r border-gray-200">0 %</td>
                    <td className="py-3 px-4">0 %</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">5 մլն. ՀՀ դրամ և ավել</td>
                    <td className="py-3 px-4 border-r border-gray-200 font-semibold">1 %</td>
                    <td className="py-3 px-4 border-r border-gray-200 font-semibold">1 %</td>
                    <td className="py-3 px-4 font-semibold">1 %</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">արտարժույթ</td>
                    <td className="py-3 px-4 border-r border-gray-200">0 %</td>
                    <td className="py-3 px-4 border-r border-gray-200">0 %</td>
                    <td className="py-3 px-4">0 %</td>
                  </tr>

                  {/* Կանխիկացման դրույքներ */}
                  <tr className="bg-gray-50/50">
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="3">
                      Քարտերով կանխիկի տրամադրում Բանկի ATM-ով, քարտային հաշվից և POS տերմինալով <sup>[5]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 font-bold block text-[10px] uppercase">ՀՀ դրամ</span>
                      Ամսական մինչև 1,000,000 ՀՀ դրամ (ներառյալ)` Անվճար,<br/>
                      1,000,000 ՀՀ դրամը գերազանցող մասի նկատմամբ` 0.2 %<br/>
                      POS տերմինալով` 0.2 %
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 font-bold block text-[10px] uppercase">ՀՀ դրամ</span>
                      MasterCard Gold՝ Ամսական մինչև 2,000,000 ՀՀ դրամ (ներառյալ)` Անվճար,<br/>
                      2,000,000 ՀՀ դրամը գերազանցող մասի նկատմամբ` 0.2 %<br/>
                      POS տերմինալով` 0.2 %<br/>
                      Visa Gold՝ 0.2 %
                    </td>
                    <td className="py-3 px-4">
                      <span className="text-gray-500 font-bold block text-[10px] uppercase">ՀՀ դրամ</span>
                      0.2%
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px] uppercase">ԱՄՆ դոլար և Եվրո</span> 0.5%
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200 text-gray-400">-</td>
                    <td className="py-3 px-4">
                      <span className="text-gray-500 block text-[10px] uppercase">ԱՄՆ դոլար և Եվրո</span> 0.5%
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px] uppercase">Այլ արտարժույթ</span> 0.3%
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200 text-gray-400">-</td>
                    <td className="py-3 px-4">
                      <span className="text-gray-500 block text-[10px] uppercase">Այլ արտարժույթ</span> 0.3%
                    </td>
                  </tr>

                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտերով կանխիկի տրամադրում «ԱրՔա» համակարգի անդամ հանդիսացող ՀՀ բանկերի ATM-ներով և POS-տերմինալներով <sup>[6]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0.8%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.8%</td>
                    <td className="py-3 px-4">0.8%</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտերով կանխիկի տրամադրում օտարերկրյա բանկերի և «ԱրՔա» համակարգի անդամ չհանդիսացող ՀՀ բանկերի ATM-ներով և POS-տերմինալներով <sup>[7]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">1% min 1,500 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1% min 1,500 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1% min 1,500 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Առևտրի կետերում Բանկի և այլ բանկերի POS տերմինալներով անկանխիկ գործարքների իրականացում
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Քարտի գործողության կասեցում</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտի գործողության ապակասեցում (սխալ PIN ծածկագրի կամ CVV մուտքագրման դեպքում)
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտի վերաթողարկում գործողության ժամկետը ավարտվելու դեպքում
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտի վերաթողարկում կորստի, վնասվելու կամ PIN ծածկագրի կորստի դեպքում
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">3,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">10,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">5,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">PIN ծածկագրի գեներացման հայտ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Քարտային հաշվի ամսական քաղվածքի տրամադրում</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտային հաշվի քաղվածքի տրամադրում 1 ամսից ավել ժամանակահատվածի համար <sup>[8]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Կանխիկի մուտքագրում «ԱրՔա» համակարգի անդամ հանդիսացող բանկերի ATM-ով (ATM CASH-IN)
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0.6 %</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.6 %</td>
                    <td className="py-3 px-4">0.6 %</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="2">
                      Գործարքի վերաբերյալ SMS հաղորդագրությունների ստացում
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">Մինչև 5,000 ՀՀ դրամ գործարքներ <sup>[9]</sup></span>
                      20 ՀՀ դրամ
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">20 ՀՀ դրամ</td>
                    <td className="py-3 px-4">20 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">5,000 ՀՀ դրամը գերազանցող գործարքներ</span>
                      0
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0</td>
                    <td className="py-3 px-4">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Գերածախսի գծով տույժեր</td>
                    <td className="py-3 px-4 border-r border-gray-200">20 % տարեկան</td>
                    <td className="py-3 px-4 border-r border-gray-200">20 % տարեկան</td>
                    <td className="py-3 px-4">20 % տարեկան</td>
                  </tr>

                  {/* Քարտից քարտ փոխանցումներ */}
                  <tr className="bg-purple-50/30">
                    <td colSpan="4" className="py-2 px-4 font-bold text-purple-900 border-b border-gray-200">
                      Քարտից քարտ փոխանցումներ (Բանկոմատների միջոցով)
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">Բանկի վճարային քարտին <sup>[10]</sup></td>
                    <td className="py-3 px-4 border-r border-gray-200">0.3%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.3%</td>
                    <td className="py-3 px-4">0.3%</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">
                      «ԱրՔա» համակարգի անդամ հանդիսացող այլ բանկերի և ԱրՔա համակարգի հետ H2H կապուղովաշխատող բանկերի վճարային քարտերին <sup>[11]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0.5%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.5%</td>
                    <td className="py-3 px-4">0.5%</td>
                  </tr>

                  <tr className="bg-purple-50/30">
                    <td colSpan="4" className="py-2 px-4 font-bold text-purple-900 border-b border-gray-200">
                      Քարտից քարտ փոխանցումներ (EvocaTouch / EvocaOnline)
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">Նույն արժույթով քարտին (Բանկի ներսում)</td>
                    <td className="py-3 px-4 border-r border-gray-200">0%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0%</td>
                    <td className="py-3 px-4">0%</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">Տարբեր արժույթով քարտերին (Բանկի ներսում)</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.3%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.3%</td>
                    <td className="py-3 px-4">0.3%</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200 pl-8">
                      Այլ բանկերի և ԱրՔա համակարգի հետ H2H կապուղովաշխատող բանկերի վճարային քարտերին
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">0.5%</td>
                    <td className="py-3 px-4 border-r border-gray-200">0.5%</td>
                    <td className="py-3 px-4">0.5%</td>
                  </tr>

                  {/* Վարկային սահմանաչափ */}
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Վարկային սահմանաչափի տրամադրման հայտի ուսումնասիրման միջնորդավճար
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">2,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">2,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">2,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Գործող վարկային սահմանաչափի ավելացման հայտի ուսումնասիրման միջնորդավճար
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Կանխիկի տրամադրման առավելագույն սահմանաչափ
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">1,000,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">2,000,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">1,000,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Կանխիկացման գործարքների օրական առավելագույն քանակ
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">10</td>
                    <td className="py-3 px-4 border-r border-gray-200">10</td>
                    <td className="py-3 px-4">10</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Բանկոմատի միջոցով կանխիկացման մեկ գործարքի առավելագույն գումարային սահմանաչափ
                    </td>
                    <td colSpan="3" className="py-3 px-4 text-center font-medium">
                      Նվազագույնը 300,000 ՀՀ դրամ (կախված է ԱԳՄ-ից)
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="3">
                      Մեկ օրվա ընթացքում կանխիկացման գործարքների կամ սահմանաչափերի քանակի ավելացման միջնորդավճար
                    </td>
                    <td colSpan="3" className="py-2 px-4 text-gray-500 font-medium text-[11px]">
                      քարտի գործողության ամբողջ ընթացքում.
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">Մինչև 5 մլն ՀՀ դրամ դեպքում</span> 2,900 ՀՀ դրամ
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">2,900 ՀՀ դրամ</td>
                    <td className="py-3 px-4">2,900 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      <span className="text-gray-500 block text-[10px]">5 մլն ՀՀ դրամ և ավելի դեպքում</span> 4,900 ՀՀ դրամ
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">4,900 ՀՀ դրամ</td>
                    <td className="py-3 px-4">4,900 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Քարտով կատարված գործարքների բողոքարկման հայտ <sup>[12]</sup>
                    </td>
                    <td className="py-3 px-4 border-r border-gray-200">5,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4 border-r border-gray-200">5,000 ՀՀ դրամ</td>
                    <td className="py-3 px-4">5,000 ՀՀ դրամ</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Ժամկետը լրացած վճարային քարտի քարտային հաշվի սպասարկման վճար
                    </td>
                    <td colSpan="3" className="py-3 px-4">
                      Ամսական 2,000 ՀՀ դրամ, իսկ միջոցների անբավարարության դեպքում՝ հաշվի մնացորդի չափով:
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>

            {/* Table 2: Այլ բանկերի քարտերի սակագներ */}
            <div className="overflow-x-auto rounded-2xl border border-gray-200/80 shadow-sm bg-white">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-purple-50/50 text-purple-900 border-b border-gray-200 font-bold">
                    <th colSpan="2" className="py-3 px-4 text-left text-xs font-bold">
                      Այլ բանկերի կողմից թողարկված վճարային քարտերի սակագներ (բացառությամբ MIR համակարգի քարտերի)
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  <tr className="bg-gray-50 font-bold text-gray-700">
                    <td className="py-3 px-4 w-1/2 border-r border-gray-200">Մատուցվող ծառայություններ</td>
                    <td className="py-3 px-4">
                      ԱրՔա համակարգի անդամ չհանդիսացող, H2H կապուղով ԱրՔա համակարգի հետ չաշխատող բանկերի կողմից թողարկված քարտերով կանխիկացում
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Բանկի բանկոմատներից կանխիկացում</td>
                    <td className="py-3 px-4 font-semibold text-purple-900">1.5 % min 1,000 ՀՀ դրամ սպասարկման միջնորդավճար</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">Բանկի POS-տերմինալներից կանխիկացում</td>
                    <td className="py-3 px-4 font-semibold text-purple-900">3 %, min 2,000 ՀՀ դրամ սպասարկման միջնորդավճար</td>
                  </tr>
                </tbody>
              </table>
            </div>

            {/* Table 3: Վիրտուալ տերմինալներ / Card-to-Card */}
            <div className="overflow-x-auto rounded-2xl border border-gray-200/80 shadow-sm bg-white">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-purple-50/50 text-purple-900 border-b border-gray-200 font-bold">
                    <th colSpan="2" className="py-3 px-4 text-left text-xs font-bold">
                      Բանկի վիրտուալ տերմինալների միջոցով տեղական՝ լոկալ, քարտից-քարտ (card-to-card) փոխանցումների սպասարկման միջնորդավճար
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  <tr className="bg-gray-50 text-gray-700 font-bold">
                    <td colSpan="2" className="py-2.5 px-4">Բանկի տերմինալների միջոցով</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 w-1/2 font-semibold border-r border-gray-200">Բանկի կողմից թողարկված քարտերով իրականացվող գործարքներ</td>
                    <td className="py-3 px-4 font-bold text-purple-900">0</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      ԱրՔա անդամ և H2H միացումով ԱրՔա պրոցեսինգային կենտրոնի հետ աշխատող բանկերի կողմից թողարկված քարտերով
                    </td>
                    <td className="py-3 px-4 font-bold text-purple-900">0.5%</td>
                  </tr>
                </tbody>
              </table>
            </div>

            {/* Table 4: Visa Direct & MoneySend */}
            <div className="overflow-x-auto rounded-2xl border border-gray-200/80 shadow-sm bg-white">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-purple-50/50 text-purple-900 border-b border-gray-200 font-bold">
                    <th colSpan="2" className="py-3 px-4 text-left text-xs font-bold">
                      Բանկի վիրտուալ տերմինալների միջոցով Visa Direct և Money Send ծառայության սպասարկման միջնորդավճար
                    </th>
                  </tr>
                  <tr className="bg-gray-50 text-gray-700 font-bold border-b border-gray-200">
                    <td colSpan="2" className="py-2.5 px-4">Բանկի Arca, Visa և Mastercard քարտերից փոխանցման սակագին</td>
                  </tr>
                  <tr className="bg-gray-100/70 text-gray-800 font-semibold border-b border-gray-200">
                    <th className="py-2.5 px-4 w-1/2 border-r border-gray-200">Visa Direct</th>
                    <th className="py-2.5 px-4">Mastercard Money Send</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  <tr>
                    <td className="py-3 px-4 border-r border-gray-200">
                      1.0 % (սպասարկման միջնորդավճար) + Բանկի տվյալ քարտի համար գործող card-to-card միջնորդավճար (նվազագույնը 1,000 ՀՀ դրամ)
                    </td>
                    <td className="py-3 px-4">
                      1.0 % (սպասարկման միջնորդավճար) + Բանկի տվյալ քարտի համար գործող card-to-card միջնորդավճար (նվազագույնը 1,000 ՀՀ դրամ)
                    </td>
                  </tr>
                  <tr className="bg-purple-50/30 font-bold text-purple-900">
                    <td colSpan="2" className="py-2.5 px-4">
                      Քարտից մեկ օրվա ընթացքում իրականացվող գործարքների սահմանաչափ և գործարքների քանակ
                    </td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="4">
                      Մեկ գործարքի առավելագույն սահմանաչափ
                    </td>
                    <td className="py-2 px-4">250,000 ՀՀ դրամ</td>
                  </tr>
                  <tr><td className="py-2 px-4">500 ԱՄՆ դոլար</td></tr>
                  <tr><td className="py-2 px-4">400 Եվրո</td></tr>
                  <tr><td className="py-2 px-4">6,000 Ռուբլի</td></tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200">
                      Մեկ հաճախորդի՝ տվյալ վճարային համակարգով գործարքների քանակի առավելագույն օրական սահմանաչափ
                    </td>
                    <td className="py-3 px-4 font-bold">5</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-semibold border-r border-gray-200" rowSpan="4">
                      Մեկ հաճախորդի՝ տվյալ վճարային համակարգով գործարքների ծավալի օրական առավելագույն սահմանաչափ
                    </td>
                    <td className="py-2 px-4">1,000,000 ՀՀ դրամ</td>
                  </tr>
                  <tr><td className="py-2 px-4">2,000 ԱՄՆ դոլար</td></tr>
                  <tr><td className="py-2 px-4">1,600 Եվրո</td></tr>
                  <tr><td className="py-2 px-4">26,000 Ռուբլի</td></tr>
                </tbody>
              </table>
            </div>

            {/* Footnotes 1 - 12 */}
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200/60 space-y-3 text-[11px] text-gray-600 leading-relaxed">
              <p><sup>[1]</sup> MasterCard Standard, Visa Classic, MasterCard Gold, Visa Gold և VISA Infinite տեսակի վճարային քարտերը տրամադրվում են ֆիզիկական անձանց, իսկ Visa Business տեսակի վճարային քարտերը՝ իրավաբանական անձանց:</p>
              <p>Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ վճարային քարտերի քարտային հաշիվներով (առանց վճարային քարտի առկայության) իրականացվող ցանկացած գործարքի համար գործում է սույն գլխով սահմանված՝ վճարային քարտերի համար սահմանված սակագները (Սակագինը կիրառվում է միայն 01/06/2018թ.-ից ներառյալ տրամադրվող վճարային քարտերի համար):</p>
              <p>Աշխատավարձային վճարային քարտերի տրամադրման և սպասարկման սակագները սահմանվում են քարտապանի գործատուի հետ կնքված պայմանագրով: Բանկի Վարչության որոշմամբ կարող են սահմանվել վճարային քարտերի տրամադրման այլ պայմաններ և սակագներ:</p>
              <p><sup>[2]</sup> Visa Gold քարտերի թողարկումը դադարեցվել է 25.11.2024թ.-ից:</p>
              <p><sup>[3]</sup> Քարտի 5 տարվա սպասարկման վճարը գանձվում է միանվագ՝ քարտի բացման պահին:</p>
              <p><sup>[4]</sup> Օտարերկրյա քաղաքացիների համար գործում է նշված սակագնի հնգապատիկը, ընդ որում լրացուցիչ քարտերը կտրամադրվեն նախապես երեք տարվա միջնորդավճարը վճարելու պայմանով:</p>
              <p><sup>[5]</sup> Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ քարտերի համար 1 %:</p>
              <p><sup>[6]</sup> Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ քարտերի համար 1.5 %:</p>
              <p><sup>[7]</sup> Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ քարտերի համար 1.5 % min 2,500 ՀՀ դրամ:</p>
              <p><sup>[8]</sup> Միջնորդավճարները ներկայացված են ներառյալ ԱԱՀ:</p>
              <p><sup>[9]</sup> Սակագինը գործում է նաև գործարքի մերժման դեպքում ուղարկված SMS հաղորդագրության համար՝ անկախ գործարքի գումարի չափից:</p>
              <p><sup>[10]</sup> Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ քարտերի համար 1%:</p>
              <p><sup>[11]</sup> Հավելյալ ժամկետով (արտոնյալ ժամանակահատվածով) վարկային սահմանաչափի (օվերդրաֆտի) առկայությամբ քարտերի համար 1.5%:</p>
              <p><sup>[12]</sup> Գանձվում է միայն այն դեպքում, երբ բողոքարկման գործընթացի արդյունքում պարզվել է, որ գործարքը կատարվել է հաճախորդի կողմից կամ հաճախորդի կողմից քարտի օգտագործման կանոնների խախտման հետևանքով:</p>
            </div>
          </div>
        )}

        <div className="my-12 space-y-4 text-xs text-gray-700 max-w-3xl leading-relaxed">
          <p>
            Եթե հաճախ ես ճամփորդում, այս քարտը քեզ համար պարզապես անհրաժեշտություն կդառնա: Դու կկարողանաս նախապես գնել քո ավիատոմսը, ամրագրել հյուրանոցային համար կամ վարձել ավտոմեքենա՝ վճարելով <strong className="text-purple-700 font-bold">Mastercard Standard</strong> քարտով:
          </p>
          <p>
            Քարտը կարող ես պատվիրել և ստանալ առանց Բանկ այցելելու: <strong className="text-purple-700 font-bold">EvocaTOUCH</strong> հավելվածի միջոցով պարզ քայլերի հաջորդականությամբ կկարողանաս պատվիրել այն՝ նշելով այն հասցեն, որտեղից կցանկանաս ստանալ քարտը: Կարևորելով մեր հաճախորդների ժամանակը՝ առաքման անվճար ծառայության միջոցով Հայաստանի ցանկացած կետում կտրամադրենք քո քարտը:
          </p>
        </div>
      </main>

      {/* Background Banner */}
      <section 
        className="w-full py-24 px-4 text-center text-white bg-cover bg-center bg-no-repeat relative flex items-center justify-center mb-16"
        style={{
          backgroundImage: `url('https://www.evoca.am/images-cache/cards/1/16131357309987/1920x527.jpg')`
        }}
      >
        <div className="max-w-4xl mx-auto relative z-10 drop-shadow-md">
          <h2 className="text-2xl md:text-3xl font-extrabold leading-tight tracking-wide">
            Ամենօրյա գնումներից մինչև անմոռանալի ճանապարհորդություն՝ քո Mastercard Standard քարտով
          </h2>
        </div>
      </section>

      {/* 4. ԱՅԼ ՔԱՐՏԵՐ SWIPER SECTION */}
      <section className="max-w-7xl mx-auto px-16 pb-20 relative">
        <h2 className="text-2xl font-bold text-gray-900 mb-8">Այլ քարտեր</h2>

        <button className="other-prev absolute left-6 top-[60%] -translate-y-1/2 z-10 text-purple-700 hover:text-purple-900 transition">
          <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15 19l-7-7 7-7" />
          </svg>
        </button>

        <button className="other-next absolute right-6 top-[60%] -translate-y-1/2 z-10 text-purple-700 hover:text-purple-900 transition">
          <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7" />
          </svg>
        </button>

        <Swiper
          modules={[Navigation]}
          navigation={{
            prevEl: '.other-prev',
            nextEl: '.other-next',
          }}
          spaceBetween={24}
          slidesPerView={1}
          breakpoints={{
            640: { slidesPerView: 2 },
            768: { slidesPerView: 3 },
            1024: { slidesPerView: 4 },
          }}
          className="w-full"
        >
         {cardsData.map((card, index) => (
  <SwiperSlide key={`${card.id}-${index}`}>
    <Link 
      to={card.path || card.link} 
      className="flex flex-col items-center text-center cursor-pointer group/card block"
    >
      <div className="w-full flex items-center justify-center mb-4 transition-transform duration-300 group-hover/card:scale-[1.03]">
        <img
          src={card.image || card.imgUrl}
          alt={card.name || card.title}
          className="w-full h-auto object-contain drop-shadow-lg"
        />
      </div>
      <h3 className="font-bold text-gray-900 text-sm md:text-base group-hover/card:text-purple-700 transition-colors">
        {card.name || card.title}
      </h3>
    </Link>
  </SwiperSlide>
))}
        </Swiper>
      </section>

      {/* 5. Օնլայն և մոբայլ բանկինգ Banner */}
      <section className="w-full bg-[#6b21a8] text-white py-12 px-6 relative overflow-hidden">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center justify-between gap-8 relative z-10">
          <div className="w-full lg:w-1/2 flex items-center justify-center gap-4">
            <div className="relative w-full max-w-[420px]">
              <div className="relative rounded-t-xl border-[8px] border-b-0 border-neutral-800 bg-neutral-900 shadow-2xl overflow-hidden aspect-[16/10]">
                <iframe
                  src="https://www.youtube.com/embed/KwAgMHEx8ys"
                  title="EvocaTOUCH Video"
                  className="w-full h-full border-0"
                  allowFullScreen
                ></iframe>
              </div>
              <div className="w-[108%] -translate-x-[4%] h-2.5 bg-neutral-700 rounded-b-lg border-t border-neutral-600 shadow-md"></div>
            </div>
          </div>

          <div className="w-full lg:w-1/2 space-y-5">
            <h2 className="text-2xl md:text-3xl font-extrabold">Օնլայն և մոբայլ բանկինգ</h2>
            <p className="text-purple-100 text-xs md:text-sm leading-relaxed">
              Evocabank-ը արագ, պարզ և նորարար ծառայություններ մատուցող բանկ է, որն առանձնանում է
              տեղեկատվական նորագույն տեխնոլոգիաների ակտիվ կիրառմամբ։
            </p>

            <div>
              <button className="bg-white text-purple-900 px-6 py-2.5 rounded-full font-bold text-xs hover:bg-purple-50 transition shadow-md">
                Դառնալ հաճախորդ
              </button>
            </div>

            <div className="flex items-center gap-4 pt-2">
              <div className="bg-white p-1 rounded-lg shadow-md shrink-0">
                <img
                  src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=https://www.evoca.am"
                  alt="QR Code"
                  className="w-16 h-16"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-purple-100">
                  Ներբեռնել հավելվածները`
                </span>
                <div className="flex items-center gap-2">
                  <img
                    src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg"
                    alt="App Store"
                    className="h-7 cursor-pointer"
                  />
                  <img
                    src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg"
                    alt="Google Play"
                    className="h-7 cursor-pointer"
                  />
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* 6. Footer Section */}
      <footer className="w-full bg-[#f8f8fa] text-[#2d2d2d] pt-10 pb-6 px-6 border-t border-gray-100 text-xs">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-end pb-4 border-b border-gray-200 gap-4">
          <div className="text-gray-400 text-[11px]">
            Թարմացվել է` 31/08/2026 17:46
          </div>
        </div>

        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 py-8">
          <div className="space-y-3">
            <div className="flex items-center gap-1">
              <span className="text-xl font-black text-purple-700 tracking-tight">evoca</span>
              <span className="text-xl font-black text-gray-800 tracking-tight">BANK</span>
            </div>
            <p className="text-gray-600 leading-relaxed text-[12px]">
              ք. Երևան, 0010,<br />
              Հանրապետության 44/2
            </p>
            <p className="text-gray-500 leading-normal text-[11px]">
              Evocabank-ը վերահսկվում է Հայաստանի Հանրապետության Կենտրոնական բանկի կողմից
            </p>
            <p className="text-[10px] text-gray-400 pt-1">
              1990 - 2026, © ԲՈԼՈՐ ԻՐԱՎՈՒՆՔՆԵՐԸ ՊԱՇՏՊԱՆՎԱԾ ԵՆ
            </p>
          </div>

          <div className="space-y-2">
            <h4 className="font-bold text-gray-900 mb-3 text-xs">Բանկի մասին</h4>
            <p className="hover:text-purple-700 cursor-pointer">Մեր մասին</p>
            <p className="hover:text-purple-700 cursor-pointer">Ղեկավարություն</p>
            <p className="hover:text-purple-700 cursor-pointer">Բաժնետերեր</p>
            <p className="hover:text-purple-700 cursor-pointer">Հաշվետվություններ</p>
            <p className="hover:text-purple-700 cursor-pointer">Իրավական ակտեր</p>
            <p className="hover:text-purple-700 cursor-pointer">Սակագներ</p>
            <p className="hover:text-purple-700 cursor-pointer">Օտարվող գույք</p>
            <p className="hover:text-purple-700 cursor-pointer">Կառուցապատողներ</p>
            <p className="hover:text-purple-700 cursor-pointer">Գործընկեր ավտոսրահներ</p>
            <p className="hover:text-purple-700 cursor-pointer">Սակագների արխիվ</p>
          </div>

          <div className="space-y-2">
            <h4 className="font-bold text-gray-900 mb-3 text-xs">Օգտակար հղումներ</h4>
            <p className="hover:text-purple-700 cursor-pointer leading-tight">
              Հաճախորդի իրավունքները (Բողոքի ներկայացման կանոններ)
            </p>
            <p className="hover:text-purple-700 cursor-pointer leading-tight">
              Հաճախորդի ռեզիդենտության չափանիշներ
            </p>
            <p className="hover:text-purple-700 cursor-pointer">Կարգավորում</p>
            <p className="hover:text-purple-700 cursor-pointer">Գաղտնիության քաղաքականություն</p>
            <p className="hover:text-purple-700 cursor-pointer">Ֆին. հաշտարար</p>
            <p className="hover:text-purple-700 cursor-pointer leading-tight">
              Ֆինանսական հանցագործությունների կանխարգելում
            </p>
            <p className="hover:text-purple-700 cursor-pointer leading-tight">
              Հղումներ Բանկի քարտապանների համար
            </p>
          </div>

          <div className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-bold text-gray-900 text-xs">Այլ հղումներ</h4>
              <div className="flex gap-2 text-gray-400 text-xs">
                <span className="hover:text-purple-600 cursor-pointer">f</span>
                <span className="hover:text-purple-600 cursor-pointer">📷</span>
                <span className="hover:text-purple-600 cursor-pointer">p</span>
                <span className="hover:text-purple-600 cursor-pointer">▶</span>
                <span className="hover:text-purple-600 cursor-pointer">in</span>
              </div>
            </div>

            <p className="hover:text-purple-700 cursor-pointer">EvocaONLINE</p>
            <p className="hover:text-purple-700 cursor-pointer">Պահատուփեր</p>
            <p className="hover:text-purple-700 cursor-pointer">Հաճախ տրվող հարցեր</p>
            <p className="hover:text-purple-700 cursor-pointer">Հայտարարություններ</p>
            <p className="hover:text-purple-700 cursor-pointer">Dibrary</p>
            <p className="hover:text-purple-700 cursor-pointer">Բուկլետներ</p>

            <div className="pt-2 space-y-1">
              <p className="text-[#6b21a8] font-bold hover:underline cursor-pointer">
                Բանկի հասցեները և աշխատաժամերը
              </p>
              <p className="text-[#6b21a8] font-bold hover:underline cursor-pointer">
                Կապ մեզ հետ
              </p>
              <p className="text-[#6b21a8] font-bold text-xs">+374 10 605555</p>
              <p className="text-[#6b21a8] font-extrabold text-sm">8444</p>
            </div>
          </div>
        </div>

        <div className="border-t border-gray-200/60 pt-4 max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] text-gray-400">
          <p className="max-w-2xl">
            Հարգելի՛ այցելու, Կայքի որևէ տեղեկատվության վերաբերյալ տարբեր լեզուներում անհամապատասխանություն...
          </p>
          <div className="flex gap-4">
            <span className="font-bold text-gray-500">arca</span>
            <span className="font-bold text-gray-500">fininfo</span>
          </div>
        </div>

        {/* Floating Chat */}
        <div className="fixed bottom-4 right-4 z-50 flex items-center">
          <button className="bg-[#6b21a8] text-white font-bold text-xs px-4 py-2 rounded-l-full shadow-lg hover:bg-purple-800 transition">
            Գրեք մեզ, մենք օնլայն ենք !
          </button>
          <div className="bg-purple-500 p-2 rounded-full text-white shadow-lg -ml-2 cursor-pointer">
            💬
          </div>
        </div>
      </footer>
    </div>
  );
};

export default PurpleCard;